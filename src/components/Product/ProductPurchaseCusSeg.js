...
function ProductPurchaseCusSeg() {
  const [product_purchaseBasedSegment, setProduct_purchaseBasedSegment] =
    useState("");
  ...
  var customer_list_object = useSelector(
    (state) => state.Product_Purchase_Based_Customer_List_and_Segment
  );
  ...
  var segs = [];
  if (
    customer_list_object.Product_Purchase_Based_Cus_Segment_Obj &&
    customer_list_object.Product_Purchase_Based_Cus_Segment_Obj.length > 0
  ) {
    segs = structuredClone(
      customer_list_object.Product_Purchase_Based_Cus_Segment_Obj
    );
  }
  ...
  var consumers = [];
  if (
    customer_list_object.Product_Purchase_Based_Cus_List_Obj &&
    customer_list_object.Product_Purchase_Based_Cus_List_Obj.length > 0
  ) {
    consumers = structuredClone(
      customer_list_object.Product_Purchase_Based_Cus_List_Obj
    );
  }
      ...
      <Grid item md={12}>
                    ...
                    render: (row) => (
                      <Button
                        onClick={() => {
                          setSegname(row.name);
                          dispatch(
                            Get_Product_Purchase_Based_Cus_List_Obj({
                              segid: row.id,
                            })
                          );
                        }}
        ...
        <form onSubmit={product_purchase_Submit}>
          <input
            name="proseg"
            type="text"
            placeholder="insert segment name to save"
            style={{
              padding: "15px 25px",
              marginBottom: "1rem",
              width: "100%",
          ...
          <div className="date-period" style={{ marginTop: "1rem" }}>
            ...
            >
              <Radio
                onClick={(e) => {
                  if (e.target.value === BillShip) setbillCity_(false);
                }}
                value="Billing_City"
                name="onsale"
              />{" "}
              ...
              <Radio
                onClick={(e) => {
                  if (e.target.value === BillShip) setshipCity_(false);
                }}
                value="Shipping_City"
                name="onsale"
              />{" "}
          ...
          <div>
            {filterList1}
            {filterList1.length > 0 && <input type="submit" value="Submit" />}
          </div>
