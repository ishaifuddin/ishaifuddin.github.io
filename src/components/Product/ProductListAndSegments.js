...
function ProductListAndSegments() {
  ...
  var Update_cog = (event) => {
    ...
    axios
      .post("https://server.shopex.io/products/product_cost_update.php", data, {
  ...
  var addfilter = (e, arg) => {
    ...
    if (arg !== 99) {
      ...
      if (arg === "Products")
        setfilterList(filterList_.concat(<Products key={"Products"} />));

      if (arg === "Catagory")
        setfilterList(filterList_.concat(<ProductCatagory key={"Catagory"} />));

      if (arg === "SKU")
        setfilterList(filterList_.concat(<ProductSKU key={"SKU"} />));

      if (arg === "Current_Price")
        setfilterList(
          filterList_.concat(<ProductCurrentPrice key={"Current_Price"} />)
        );

      if (arg === "Height")
        setfilterList(filterList_.concat(<ProductHeight key={"Height"} />));

      if (arg === "Width")
        setfilterList(filterList_.concat(<ProductWidth key={"Width"} />));

      if (arg === "Length")
        setfilterList(filterList_.concat(<ProductLength key={"Length"} />));

      if (arg === "Weight")
        setfilterList(filterList_.concat(<ProductWeight key={"Weight"} />));

      if (arg === "Stock")
        setfilterList(filterList_.concat(<ProductStock key={"Stock"} />));

      if (arg === "on_sale")
        setfilterList(filterList_.concat(<ProductOnSale key={"on_sale"} />));

      if (arg === "Type")
        setfilterList(filterList_.concat(<ProductType key={"Type"} />));
    }
  };
          ...
          <ThemeProvider theme={defaultMaterialTheme}>
            {
              ...
              <MaterialTable
                ...
                onRowClick={(event, rowData) => {
                  dispatch(
                    get_products_from_selected_catagory({
                    ...
                    render: (row) => (
                      <>
                        ...
                        <input
                          form="cogform"
                          type="submit"
                          style={{ padding: "2px 12px", fontWeight: "600" }}
                          value="Save-COG"
                        />{" "}
                      </>
                    ),
