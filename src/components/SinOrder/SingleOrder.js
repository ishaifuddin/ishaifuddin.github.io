...
function SingleOrder() {
    ...
    useEffect(() => {
        ...
        const sinOrder = async () => {
            try {
                const res = await axios.post('https://server.shopex.io/orders/single-order/single_order.php', { oid: chc }, { withCredentials: true });
    ...
    var items_columns = [
        { title: 'Product', field: 'name', render: row => <div style={{ background: 'mintcream', fontFamily: 'system-ui', fontSize: '16px', textAlign: 'left' }}>  {row.name} </div> },
    ...
    var sum_columns = [
        { title: 'Discount', field: 'discount', render: row => <div style={{ background: 'mintcream', fontFamily: 'system-ui', fontSize: '16px', textAlign: 'left' }}>  {row.discount} </div> },
        { title: 'ShipCost', field: 'shipcost', render: row => <div style={{ background: 'ghostwhite' }}>  {row.shipcost} </div> },
        { title: 'Tax', field: 'tax', render: row => <div style={{ background: 'ghostwhite' }}>  {row.tax} </div> },
        { title: 'Grand Total', field: 'amount', render: row => <div style={{ background: 'ghostwhite' }}>  {row.amount} </div> }
    ...
    return (
        ...
        <div>
            ...
            <Grid container style={{ "background":"mediumseagreen", "width": "82%", "margin": "1%", "borderRadius": "14px",  "boxShadow": "rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px" }}>
                <ShoppingCartIcon style={{ "fontSize": "60px", "color": "white", "padding": "6px", "marginTop": "3px", marginLeft: '1%' }} />
                <span style={{paddingTop:'10px',color:'white'}}>
                    <h5> Order ::  {chc}</h5>
                    <p>{shipcity},{shipstate},{sa1},{sa2}</p>
                </span>
            </Grid>
                    ...
                    <Timeline
                        ...
                        <TimelineItem style={{ minHeight: '57px' }}>
                            ...
                            <TimelineContent style={timelinecontentstyle}>
                                ...
                                <Typography> <a href={'/Customers/profile/' + cusCHC}> {mail}</a> </Typography>
                            </TimelineContent>
