...
function Orders() {
  ...
  useEffect(() => {
    axios
      .post(
        "https://server.shopex.io/customers/single-customer/SingleCustomer_orders.php",
        {
  ...
  var columns = [
    {
      ...
      field: "date",
      render: (row) => (
        <div style={{ background: "whitesmoke" }}> {row.date} </div>
      ),
    },
    {
      title: "Id",
      ...
      render: (row) => (
        <div style={{ background: "ghostwhite" }}>
          ...
          <strong>
            {" "}
            <a href={"/Orders/" + row.orderid}> {row.orderid} </a>{" "}
          </strong>{" "}
        </div>
      ),
    },
    {
      ...
      field: "order_status",
      render: (row) => (
        <div style={{ background: "whitesmoke" }}> {row.order_status} </div>
      ),
    },
    {
      title: "Amotal",
      field: "order_total",
      render: (row) => (
        <div style={{ background: "ghostwhite" }}> {row.order_total} </div>
      ),
    },
    {
      title: "Total product",
      field: "total_product",
      render: (row) => (
        <div style={{ background: "whitesmoke" }}> {row.total_product} </div>
      ),
    },
    {
      title: "Total unit",
      field: "total_unit",
      render: (row) => (
        <div style={{ background: "ghostwhite" }}> {row.total_unit} </div>
      ),
    },
  ];
  ...
  var orderbyday_columns = [
    {
      ...
      field: "day",
      render: (row) => (
        <div style={{ background: "whitesmoke" }}> {row.day} </div>
      ),
    },
    {
      title: "Order",
      field: "order",
      render: (row) => (
        <div style={{ background: "ghostwhite" }}> {row.order} </div>
      ),
    },
  ];
