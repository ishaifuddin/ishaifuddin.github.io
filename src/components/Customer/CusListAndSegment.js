...
function CusListAndSegment() {
    ...
    var addfilter = (e, arg) => {
        ...
        if (arg !== 99) {
            ...
            if (arg === "Spend") setfilterList(filterList.concat(<Spends key={'Spend'} />));

            if (arg === "Aov") setfilterList(filterList.concat(<Aov_ key={'Aov'} />));

            if (arg === "Order_Count") setfilterList(filterList.concat(<Order_Count_ key={'Order_Count'} />));

            if (arg === "Item_Count") setfilterList(filterList.concat(<Item_Count_ key={'Item_Count'} />));

            if (arg === "Joined") setfilterList(filterList.concat(<Joined_ key={'Joined'} />));

            if (arg === "First_Ordered") setfilterList(filterList.concat(<First_Ordered_ key={'First_Ordered'} />));

            if (arg === "Last_Ordered") setfilterList(filterList.concat(<Last_Ordered_ key={'Last_Ordered'} />));

            if (arg === "Average_Day_Between_order") setfilterList(filterList.concat(<Dbet_ret_ key={'Average_Day_Between_order'} />));

            if (arg === "Order_From_Offer") setfilterList(filterList.concat(<Order_From_Offer_ key={'Order_From_Offer'} />));

            if (arg === "Total_Profit") setfilterList(filterList.concat(<Total_Profit_ key={'Total_Profit'} />));

            if (arg === "Total_Discount") setfilterList(filterList.concat(<Total_Discount_ key={'Total_Discount'} />));

            if (arg === "Coupon_Use") setfilterList(filterList.concat(<Coupon_Use_ key={'Coupon_Use'} />));

            if (arg === "Billing_City") setfilterList(filterList.concat(<Billing_City_ key={'Billing_City'} />));

            if (arg === "Billing_Country") setfilterList(filterList.concat(<Billing_Country_ key={'Billing_Country'} />));

            if (arg === "Shipping_City") setfilterList(filterList.concat(<Shipping_City_ key={'Shipping_City'} />));

            if (arg === "Shipping_Country") setfilterList(filterList.concat(<Shipping_Country_ key={'Shipping_Country'} />));

            if (arg === "Shipping_Cost") setfilterList(filterList.concat(<Shipping_Cost_ key={'Shipping_Cost'} />));

            if (arg === "Customer_Source") setfilterList(filterList.concat(<Customer_Source_ key={'Customer_Source'} />));

            if (arg === "Customer_Tag") setfilterList(filterList.concat(<Customer_Tag_ key={'Customer_Tag'} />));

            if (arg === "Avg_Shipping_Cost") setfilterList(filterList.concat(<Avg_Shipping_Cost_ key={'Avg_Shipping_Cost'} />));
            if (arg === "Avg_Discount") setfilterList(filterList.concat(<Avg_Discount_ key={'Avg_Discount'} />));
    ...
    var columns = [
        ...
        { title: 'Joined', field: 'reg', render: row => <div style={{ background: 'whitesmoke' }}>  {row.reg} </div> },
        { title: 'FirstOrder', field: 'fod', render: row => <div style={{ background: 'ghostwhite' }}>  {row.fod} </div> },
        { title: 'LastOrder', field: 'lod', render: row => <div style={{ background: 'whitesmoke' }}>  {row.lod} </div> },
        { title: 'Order', field: 'total_order', render: row => <div style={{ background: 'ghostwhite' }}>  {row.total_order} </div> },
        { title: 'Av.DayGap', field: 'adbo', render: row => <div style={{ background: 'whitesmoke' }}>  {row.adbo} </div> },
        { title: 'Spent', field: 'total_spent', render: row => <div style={{ background: 'ghostwhite' }}>  {row.total_spent} </div> },
        { title: 'Discount', field: 'total_discount', render: row => <div style={{ background: 'whitesmoke' }}>  {row.total_discount} </div> },
        { title: 'AOV', field: 'aov', render: row => <div style={{ background: 'ghostwhite' }}> {row.aov} </div> },
