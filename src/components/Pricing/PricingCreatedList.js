...
function PricingCreatedList() {
  ...
  var product_obj = useSelector(
    (state) => state.product_List_And_Segments.product_table_object
  );
  ...
  var product_cat_obj = useSelector(
  ...
  var EditRule = (Name, Type) => {
    axios
      .post(
        "https://server.shopex.io/dynamicpricing/dpp_single_pricing_rule.php",
        {
          name: Name,
          ajax_call: "2",
        },
      ...
      .then(function (response) {
        ...
        if (ruletype === "q_dis") {
              ...
              if (product_obj && product_obj.length > 0) {
                for (var j of product_obj) {
                  if (offer_on_id === j.product_id) {
                    var found = Qdis_AvailableForProduct.find(
                      (element) => element.value === offer_on_id
                    );
                    if (found === undefined) {
              ...
              if (product_cat_obj && product_cat_obj.length > 0) {
                for (var j of product_cat_obj) {
                  if (offer_on_id === j.catagory_id) {
                    var found = Qdis_AvailableForCategory.find(
                      (element) => element.value === offer_on_id
                    );
                    if (found === undefined) {
          ...
          var Product_or_Category =
            response.data.onerule[0].dtail.split("_next_")[0];
            ...
            columns={[
              {
                ...
                field: "status",
                render: (row) => (
                  <div style={{ background: "whitesmoke" }}> {row.status} </div>
                ),
              },
              {
                title: "Name",
                field: "name",
                render: (row) => (
                  <div style={{ background: "ghostwhite" }}> {row.name} </div>
                ),
              },
              {
                title: "Type",
                field: "type",
                render: (row) => (
                  <div style={{ background: "whitesmoke" }}> {row.type} </div>
                ),
              },
              {
