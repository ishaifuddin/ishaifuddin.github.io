...
function PricingCreate() {
    ...
    var discount_type_change = (e) => {
        ...
        if(distype=='Quantity')setfilterList((<Quantity key={'Quantity'} target_segment_name={""}target_segment_id={""}offername={""}schedule={""}osrun={""}status={""}pr={""}qt={""}for_category={""}for_product={""}offer_on_pro_or_cat={""} />));  
        else if(distype=='GiftProduct')setfilterList((<GiftProduct key={'GiftProduct'} />));  
        else if(distype=='DiscountOnEntireShop')setfilterList((<DiscountOnEntireShop key={'DiscountOnEntireShop'} />));  
        else if(distype=='CategoryDiscount')setfilterList((<CategoryDiscount key={'CategoryDiscount'} />));  
    }
