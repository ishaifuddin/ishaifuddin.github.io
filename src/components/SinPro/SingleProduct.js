...
function SingleProduct() {
    ...
    useEffect(() => {
        ...
        const sinpro_card = async () => {
            try {
                const res = await axios.post('https://server.shopex.io/products/single-product/sinpro_card.php', { product_chc: chc }, { withCredentials: true });
        ...
        const sinpro_cuslist = async () => {
            try {
                const res = await axios.post('https://server.shopex.io/products/single-product/sinpro_cuslist.php', { product_chc: chc }, { withCredentials: true });
                setCustomers(res.data.customers);
            } catch { }
        };
        sinpro_cuslist();
        ...
        const sinpro_bought_with = async () => {
            try {
                const res = await axios.post('https://server.shopex.io/products/single-product/sinpro_bought_with.php', { product_chc: chc }, { withCredentials: true });
        ...
        const avg_sales = async () => {
            try {
                const res = await axios.post('https://server.shopex.io/products/single-product/single-product-month-quarter-avg-sales.php', { product_chc: chc }, { withCredentials: true });
        ...
        const retention = async () => {
            try {
                const res = await axios.post('https://server.shopex.io/products/single-product/sinpro_retention.php', { product_chc: chc }, { withCredentials: true });
    ...
    var customer_columns = [{
        title: 'Customer', render: row =>
            <div style={{ background: 'mintcream', fontFamily: 'system-ui', fontSize: '16px', textAlign: 'left' }}>
                <a href={'/Customers/profile/' + row.customer_chc}> {row.customer_name}</a>
            </div>
    },
    ...
    var fbw_columns = [
    { title: 'Product', render: row => <div style={{ background: 'mintcream', fontFamily: 'system-ui', fontSize: '16px', textAlign: 'left' }}>{row.name} </div>},
    { title: 'Freq', render: row => <div style={{ background: 'whitesmoke' }}>  {row.buy} </div> }];


    var month_columns = [
    { title: 'Month', fiend: 'month', render: row => <div style={{ background: 'mintcream', fontFamily: 'system-ui', fontSize: '16px', textAlign: 'left' }}>{row.month} </div>},
    { title: 'Avg rev', fiend: 'rev', render: row => <div style={{ background: 'whitesmoke' }}>  {row.rev} </div> },
    { title: 'Avg unit', fiend: 'unit', render: row => <div style={{ background: 'whitesmoke' }}>  {row.unit} </div> }];

    var quarter_columns = [
    { title: 'Quarter',fiend:'quarter', render: row =>  <div style={{ background: 'mintcream', fontFamily: 'system-ui', fontSize: '16px', textAlign: 'left' }}>{row.quarter} </div>},
        ...
        <Grid container>
            ...
            <Grid container style={{ "background": "rgb(52, 195, 255)", "width": "82%", "margin": "1%", "borderRadius": "50px", "padding": "4px", "boxShadow": "rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px" }}>
                <Grid style={{ display: 'inline-flex' }}>
                    ...
                    <div style={{}}>
                        <h5 style={{ color: 'white', padding: '10px', marginTop: '12px' }}>{name}</h5>
                    </div>
                </Grid>
            </Grid>
                ...
                <Grid item sm={10}>
                    ...
                    <Grid container>
                        ...
                        <Grid item sm={11} style={{marginTop:'2%'}}>
                            ...
                            <h5 style={{"marginTop":"7%","marginBottom":'2%',"width":"97%","background":"ghostwhite","padding":"2%"}}>Retention Data</h5>
                            
                            <h5 style={{"border-top": "0.1px solid darkgray","marginTop":'20px',"background":"white","boxShadow":"rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px","borderRadius":"3px","padding":"10px"}}>Customer retention</h5>
                            <Grid container style={{"background":"whitesmoke","boxShadow":"rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px"}}>
                                <div style={{  width:'-webkit-fill-available' }} dangerouslySetInnerHTML={{ __html: cus }}></div>
                            </Grid>

                            <h5 style={{"border-top": "0.1px solid darkgray","marginTop":'20px',"background":"white","boxShadow":"rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px","borderRadius":"3px","padding":"10px"}}>Revenue generated</h5>
                            <Grid container style={{"background":"whitesmoke","boxShadow":"rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px"}}>
                                <div style={{  width:'-webkit-fill-available' }} dangerouslySetInnerHTML={{ __html: rev }}></div>
                            </Grid>

                            <h5 style={{"border-top": "0.1px solid darkgray","marginTop":'20px',"background":"white","boxShadow":"rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px","borderRadius":"3px","padding":"10px"}}>Unit sold </h5>
                            <Grid container style={{"background":"whitesmoke","boxShadow":"rgb(65 69 88 / 10%) 0px 7px 14px 0px, rgb(0 0 0 / 7%) 0px 3px 6px 0px"}}>
