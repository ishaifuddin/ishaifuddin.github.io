...
export const get_single_city_all_product_data = createAsyncThunk(
  "product/single_city_performance",
  async (data) => {
    try {
      await axios
        .post(
          "https://server.shopex.io/products/products_sincity_perform.php",
          data,
...
export const Product_specific_city_all_product_Slice = createSlice({
  ...
  extraReducers: {
    [get_single_city_all_product_data.pending]: (state, action) => {
      state.status = "loading";
    },
    [get_single_city_all_product_data.fulfilled]: (state, action) => {
      state.status = "success";

      if (action.payload !== null) {
        //// Charts ////

        state.single_city_comparison_chart_base_label = action.payload.label;
        state.single_city_comparison_table_object =
          action.payload.comparison_table;
    ...
    [get_single_city_all_product_data.rejected]: (state, action) => {
      state.status = "failed";
    },
  },
});
